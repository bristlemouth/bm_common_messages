...
static_assert(
    offsetof(struct sensor_header_msg_data, version) ==
            offsetof(SensorHeaderMsg::Data, version) &&
        offsetof(struct sensor_header_msg_data, reading_time_utc_ms) ==
            offsetof(SensorHeaderMsg::Data, reading_time_utc_ms) &&
        offsetof(struct sensor_header_msg_data, reading_uptime_millis) ==
            offsetof(SensorHeaderMsg::Data, reading_uptime_millis) &&
        offsetof(struct sensor_header_msg_data, sensor_reading_time_ms) ==
            offsetof(SensorHeaderMsg::Data, sensor_reading_time_ms) &&
...
#define BOREALIS_LEVELS_MSG_NUM_FIELDS (3 + SensorHeaderMsg::NUM_FIELDS)
...
#define BOREALIS_RECORDING_STATUS_MSG_NUM_FIELDS                               \
  (4 + SensorHeaderMsg::NUM_FIELDS)
...
#define BOREALIS_LEVEL_STATISTICS_MSG_VERSION 1
#define BOREALIS_LEVEL_STATISTICS_MSG_NUM_FIELDS                               \
  (5 + SensorHeaderMsg::NUM_FIELDS)

struct borealis_level_statistics {
#ifdef __cplusplus
  SensorHeaderMsg::Data header;
#else
  struct sensor_header_msg_data header;
...
extern "C" {
#endif
CborError borealis_spectrum_data_encode(struct borealis_spectrum_data *d,
                                        uint8_t *cbor_buffer, size_t size,
                                        size_t *encoded_len);
CborError borealis_spectrum_data_decode(struct borealis_spectrum_data *d,
                                        uint8_t *cbor_buffer, size_t size);

CborError borealis_levels_encode(struct borealis_levels *d,
                                 uint8_t *cbor_buffer, size_t size,
                                 size_t *encoded_len);
CborError borealis_levels_decode(struct borealis_levels *d,
                                 uint8_t *cbor_buffer, size_t size);

CborError borealis_recording_status_encode(struct borealis_recording_status *d,
                                           uint8_t *cbor_buffer, size_t size,
                                           size_t *encoded_len);
CborError borealis_recording_status_decode(struct borealis_recording_status *d,
                                           uint8_t *cbor_buffer, size_t size);

CborError borealis_levels_statistics_encode(struct borealis_level_statistics *d,
                                            uint8_t *cbor_buffer, size_t size,
                                            size_t *encoded_len);
CborError borealis_levels_statistics_decode(struct borealis_level_statistics *d,
                                            uint8_t *cbor_buffer, size_t size);
