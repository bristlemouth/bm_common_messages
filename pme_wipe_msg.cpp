...
CborError encode(Data &w, uint8_t *cbor_buffer, size_t size,
  ...
  do {
    ...
    err = cbor_encode_double(&map_encoder, w.avg1_mA);
    if (err != CborNoError) {
    ...
    err = cbor_encode_double(&map_encoder, w.avg2_mA);
    if (err != CborNoError) {
...
CborError decode(Data &w, const uint8_t *cbor_buffer, size_t size) {
  ...
  do {
    ...
    err = cbor_value_get_double(&value, &w.avg1_mA);
    if (err != CborNoError) {
      break;
    }
    err = cbor_value_advance(&value);
    ...
    err = cbor_value_get_double(&value, &w.avg2_mA);
    if (err != CborNoError) {
      break;
    }
    err = cbor_value_advance(&value);
